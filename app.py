...
if not df_raw.empty:
    ...
    if not df.empty:
        ...
        # Berechnung der Abstände für die Grafiken
        ...
        # 2. NEU: Durchschnittlicher Abstand nach Jahr
        st.markdown("### Durchschnittlicher Abstand nach Jahr (in Tagen)")
        # Wir gruppieren nach Jahr und berechnen den Mittelwert der Spalte 'Abstand'
        ...
        if not yearly_avg['Abstand'].dropna().empty:
            ...
            st.info("Nicht genügend Daten für Abstands-Analyse pro Jahr.")

        # 3. Häufigkeit nach Wochentag
        st.markdown("### Häufigkeit nach Wochentag")
        w_order = ['Mo', 'Di', 'Mi', 'Do', 'Fr', 'Sa', 'So']
        wd_counts = df['Wochentag'].value_counts().reindex(w_order
